...
test:
	@ make clean
	@ dune build @all
	@ make -C test/HashSet random

.PHONY: bench
bench:
	@ dune build @all
	@ make -C benchmark/HashSet bench
...
ASSEMBLY=$(shell find . -name "hachis__Hachis.s")
.PHONY: assembly
assembly:
...
HEADACHE := headache
HEADER   := header.txt
SOURCES  := src test benchmark

.PHONY: headache
headache:
	@ for f in $(shell gfind $(SOURCES) -type f -regex ".*\.mli?") ; do \
